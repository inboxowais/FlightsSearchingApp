    ...
    if (action.url) {
        try {
            ...
            if (action.method == 'POST' || action.method == 'PUT') {
                ...
                else if (action.encoded == "MULTI_PART_FORM") {
                    data = new FormData();
            ...
            if (response &&
                (response.code == 812 || response.code == 900 || response.code == 801 || response.code == 802 || response.code == 804 || response.code == 800
                    || response.code == 816 || response.code == 803 || response.code == 806 || response.code == 807 || response.code == 811)) {
                yield put({ type: `${action.type}_ERROR`, error: { errorId: `${action.type}_ERROR_ID` } });
            }
            else if (response) {
                yield put({ type: `${action.type}_SUCCESS`, baseType: `${action.type}`, response, data: action.data });
            }
